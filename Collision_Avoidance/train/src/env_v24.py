...
class Test(core.Env):
    ...
    def move(self, goal):
        ...
        self.max_force = np.linalg.norm(s[8:10])
        self.max_torque = np.linalg.norm(s[11:13])
        self.state = s
        return s
    ...
    def reset(self):
        ...
        # print(self.get_force_sensor('r').shape)
        ...
        self.max_force = np.linalg.norm(self.state[8:10])
        self.max_torque = np.linalg.norm(self.state[11:13])
        self.done = False
        self.success = False
        return self.state

    def set_goal(self):
        # self.goal = np.array([0.0, 55.36, -6.36, 0.0, 78.91, 56.61, -80.19, -14.50])*(np.pi/180)# [0.33, 0.0, -0.56, 0, 0, 0, 1]
        self.goal = np.array([0.0, 56.67, -6.87, 0.0, 80.83, 57.71, -81.27, -13.51])*(np.pi/180)# [0.33, 0.0, -0.58, 0, 0, 0, 1]
        # self.goal = np.array([0.0, 55.48, -6.40, 0.0, 79.09, 56.71, -80.29, -14.41])*(np.pi/180)# [0.33, 0.0, -0.59, 0, 0, 0, 1]
    ...
    def step(self, a):        
        ...
        if res.success:
            ...
            self.dis_z = np.linalg.norm(s[2] + 0.56) #abs?
            self.theta_z = np.linalg.norm(self.goal[2] - s[2])
            self.dis_pos = np.linalg.norm(self.goal[:3] - s[:3])
    ...
    def get_reward(self, s, ik_success, terminal, singularity):
        ...
        elif self.max_force > 10:
            reward -= 5
        
        elif self.max_force > 0:
            reward -= 1

        if self.max_torque > 5:
            reward -= 50

        elif self.max_torque > 4:
            reward -= 40
